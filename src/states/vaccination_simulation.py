...
class VaccinationSimulation:
    def __init__(self, screen):
        ...
        self.INFECTION_PROB = 0.15
        self.RECOVERY_TIME = 400
        self.SPEED = 2.0
        self.VACCINATION_RATE = 0.7  # 70% vaccination rate
        self.VACCINE_EFFECTIVENESS = 0.95  # 95% effective
        ...
        # Assign vaccination status to nodes
        for node in self.nodes:
            node['vaccinated'] = random.random() < self.VACCINATION_RATE
                ...
                for j, node2 in enumerate(self.nodes):
                    if i != j and node2['status'] == 'healthy':
                        ...
                        if distance < self.INFECTION_RADIUS:
                            ...
                            # Apply vaccine effectiveness if vaccinated
                            if node2['vaccinated']:
                                effective_prob *= (1 - self.VACCINE_EFFECTIVENESS)
        ...
        for node in self.nodes:
            if node['status'] == 'infected':
                ...
                if node['infection_timer'] > self.RECOVERY_TIME:
                    # Vaccination: 95% recovery rate, 5% death rate
    ...
    def draw(self):
        ...
        for node in self.nodes:
            ...
            # Draw vaccination indicator for healthy nodes
            if node['status'] == 'healthy' and node['vaccinated']:
                pygame.draw.circle(self.screen, (255, 215, 0), [int(p) for p in pos], 5, 1)  # Gold ring
