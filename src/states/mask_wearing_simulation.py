...
class MaskWearingSimulation:
    def __init__(self, screen):
        self.screen = screen
        self.nodes, self.edges = load_network("data/graphs/network.json")
        
        # Mask Wearing specific parameters
        self.INFECTION_RADIUS = 15
        self.INFECTION_PROB = 0.3
        self.RECOVERY_TIME = 900
        self.SPEED = 1.8
        self.MASK_EFFECTIVENESS = 0.9
        self.MASK_WEARING_RATE = 0.7  # 70% of people wear masks
        ...
        for node in self.nodes:
            if node['status'] == 'infected':
                ...
                if node['infection_timer'] > self.RECOVERY_TIME:
                    # Mask Wearing: 70% recovery rate, 30% death rate
