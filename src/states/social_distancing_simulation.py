...
class SocialDistancingSimulation:
    def __init__(self, screen):
        self.screen = screen
        self.nodes, self.edges = load_network("data/graphs/network.json")
        
        # Social Distancing specific parameters
        self.INFECTION_RADIUS = 8
        self.INFECTION_PROB = 0.4
        self.RECOVERY_TIME = 1000
        self.SPEED = 1.5
        self.SOCIAL_DISTANCING_FACTOR = 0.9
                ...
                for j, node2 in enumerate(self.nodes):
                    if i != j and node2['status'] == 'healthy':
                        ...
                        if distance < self.INFECTION_RADIUS:
                            ...
                            effective_prob = self.INFECTION_PROB * (1 - self.SOCIAL_DISTANCING_FACTOR)
        ...
        for node in self.nodes:
            if node['status'] == 'infected':
                ...
                if node['infection_timer'] > self.RECOVERY_TIME:
                    # Social Distancing: 60% recovery rate, 40% death rate
